...
class SaverMixin:
    ...
    def get_rgb_image_(self, img, data_format, data_range):
        ...
        imgs = [
            img[..., start : start + 3] for start in range(0, img.shape[-1], 3)
        ]
        imgs = [
            (
                img_
    ...
    def get_image_grid_(self, imgs):
        if isinstance(imgs[0], list):
            return np.concatenate(
                [self.get_image_grid_(row) for row in imgs], axis=0
            )
        cols = []
        for col in imgs:
            ...
            if col["type"] == "rgb":
                ...
                cols.append(
                    self.get_grayscale_image_(col["img"], **grayscale_kwargs)
                )
    ...
    def save_cubemap(self, filename, img, data_range=(0, 1)):
        ...
        for start in range(0, img.shape[-1], 3):
            ...
            img_full = np.concatenate(
                [
                    np.concatenate(
                        [placeholder, img_[2], placeholder, placeholder],
                        axis=1,
                    ),
                    np.concatenate(
                        [img_[1], img_[4], img_[0], img_[5]], axis=1
                    ),
                    np.concatenate(
                        [placeholder, img_[3], placeholder, placeholder],
                        axis=1,
                    ),
                ],
                axis=0,
    ...
    def save_mesh(
        ...
        v_pos, t_pos_idx = self.convert_data(v_pos), self.convert_data(
            t_pos_idx
        )
