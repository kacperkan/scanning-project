        ...
        if os.path.isdir(self.save_root):
            for f in os.listdir(self.save_root):
                ...
                if bn.startswith("version_"):
                    dir_ver = (
                        os.path.splitext(bn)[0].split("_")[1].replace("/", "")
                    )
    ...
    def save_code_snapshot(self):
        ...
        for f in self.get_file_list():
            if not os.path.exists(f) or os.path.isdir(f):
                continue
            os.makedirs(
                os.path.join(self.savedir, os.path.dirname(f)), exist_ok=True
            )
...
class ConfigSnapshotCallback(VersionedCallback):
    ...
    def save_config_snapshot(self):
        ...
        shutil.copyfile(
            self.config.cmd_args["config"],
            os.path.join(self.savedir, "raw.yaml"),
        )
