...
def normalize_poses(poses, pts, up_est_method, center_est_method):
    if center_est_method == "camera":
        ...
        center = (
            torch.stack([cams_dir, cams_dir.roll(1, 0)], dim=-1)
            * t[:, None, :]
...
def create_spheric_poses(cameras, n_steps=120):
    ...
    up = torch.as_tensor(
        [0.0, 0.0, 1.0], dtype=center.dtype, device=center.device
    )
    ...
    for theta in torch.linspace(0, 2 * math.pi, n_steps):
        ...
        c2w = torch.cat(
            [torch.stack([s, u, -l], dim=1), cam_pos[:, None]], axis=1
        )
        all_c2w.append(c2w)
        ...
        if not ColmapDatasetBase.initialized:
            ...
            for i, d in enumerate(imdata.values()):
                ...
                c2w = torch.from_numpy(
                    np.concatenate([R.T, -R.T @ t], axis=1)
                ).float()
                c2w[:, 1:3] *= -1.0  # COLMAP => OpenGL
                all_c2w.append(c2w)
                if self.split in ["train", "val"]:
                    img_path = os.path.join(
                        self.config.root_dir, "images", d.name
                    )
                    ...
                    if has_mask:
                        ...
                        mask = Image.open(mask_paths[0]).convert(
                            "L"
                        )  # (H, W, 1)
            ...
            pts3d = read_points3d_binary(
                os.path.join(self.config.root_dir, "sparse/0/points3D.bin")
            )
            pts3d = torch.from_numpy(
                np.array([pts3d[k].xyz for k in pts3d])
            ).float()
            all_c2w, pts3d = normalize_poses(
                all_c2w,
                pts3d,
