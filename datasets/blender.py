...
class BlenderDatasetBase:
    def setup(self, config, split):
        ...
        with open(
            os.path.join(
                self.config.root_dir, f"transforms_{self.split}.json"
            ),
            "r",
        ) as f:
            meta = json.load(f)
        ...
        if "img_wh" in self.config:
            ...
            w, h = (
                W // self.config.img_downscale,
                H // self.config.img_downscale,
            )
        else:
            raise KeyError(
        ...
        for i, frame in enumerate(meta["frames"]):
            ...
            img_path = os.path.join(
                self.config.root_dir, f"{frame['file_path']}.png"
            )
    ...
    def setup(self, stage=None):
        ...
        if stage in [None, "fit", "validate"]:
            self.val_dataset = BlenderDataset(
                self.config, self.config.val_split
            )
        if stage in [None, "test"]:
            self.test_dataset = BlenderDataset(
                self.config, self.config.test_split
            )
        if stage in [None, "predict"]:
            self.predict_dataset = BlenderDataset(
                self.config, self.config.train_split
            )
