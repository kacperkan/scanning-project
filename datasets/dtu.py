...
def create_spheric_poses(cameras, n_steps=120):
    ...
    cam_center = F.normalize(cameras.mean(0), p=2, dim=-1) * cameras.mean(
        0
    ).norm(2)
    ...
    max_angle = (
        (
            F.normalize(cameras, p=2, dim=-1)
            * F.normalize(cam_center, p=2, dim=-1)
        )
        .sum(-1)
        .acos()
        .max()
...
class DTUDatasetBase:
    def setup(self, config, split):
        ...
        cams = np.load(
            os.path.join(self.config.root_dir, self.config.cameras_file)
        )

        img_sample = cv2.imread(
            os.path.join(self.config.root_dir, "image", "000000.png")
        )
        ...
        for i in range(n_images):
            ...
            if self.split in ["train", "val"]:
                img_path = os.path.join(
                    self.config.root_dir, "image", f"{i:06d}.png"
                )
