...
class BaseImplicitGeometry(BaseModel):
    ...
    def isosurface_(self, vmin, vmax):
        ...
        level = chunk_batch(
            batch_func,
            self.config.isosurface.chunk,
            True,
            self.helper.grid_vertices(),
        )
...
class VolumeSDF(BaseImplicitGeometry):
    ...
    def forward(
        ...
        if with_laplace:
            assert (
                self.config.grad_type == "finite_difference"
            ), "Laplace computation is only supported with grad_type='finite_difference'"
            rv.append(laplace)
