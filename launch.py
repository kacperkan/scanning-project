...
def main():
    ...
    config.exp_dir = config.get("exp_dir") or os.path.join(
        args.exp_dir, config.name
    )
    ...
    system = systems.make(
        ...
        load_from_checkpoint=(
            None if not args.resume_weights_only else args.resume
        ),
    )
    ...
    if args.train:
        loggers += [
            ...
            CSVLogger(
                config.exp_dir, name=config.trial_name, version="csv_logs"
            ),
        ]
